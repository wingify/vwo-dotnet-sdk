...
namespace VWOSdk.Tests
{
    public class Murmur32HasherTests
    {
        ...
        [Theory]
        #region InlineData For isBucketingSeed True      
        [InlineData(1, "Anup", 10000, 1, 9667, 4151653230, 0.96663209376856685)]
        [InlineData(3, "Varun", 10000, 1, 7007, 3009464184, 0.70069548301398754)]
        [InlineData(4, "Sanyam", 10000, 1, 9119, 3916446988, 0.911868873052299)]
        #endregion InlineData
        public void MurmurHash32_Compute_Should_Return_Same_Hash_For_A_Particular_User_And_Campaign_IsBucketingSeed_True(int campaignId, string userId, double maxVal, double multiplier, double expectedHash, long murmur, double ratio)
        {
            double hash = _hasher.ComputeBucketValue(campaignId.ToString() + "_" + userId, userId, maxVal, multiplier);
            Assert.True(hash >= 0);
            Assert.True(hash <= maxVal);
            Assert.Equal(expectedHash.ToString(), hash.ToString());//Test data was generated by converting computed hash into string, hence asserting on string
        }
