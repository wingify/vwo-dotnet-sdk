    ...
    internal class SettingsProcessor : ISettingsProcessor
    {
        ...
        public AccountSettings ProcessAndBucket(Settings settings)
        {
            try
            {
                ...
                return new AccountSettings(settings.SdkKey, campaigns,  settings.AccountId, settings.Version,settings.Groups, settings.CampaignGroups, settings.IsEventArchEnabled, settings.collectionPrefix, settings.isNB, settings.isNBv2);
            }
        ...
        {
            return new BucketedCampaign(campaign.Id, campaign.Name, campaign.PercentTraffic, campaign.Key, campaign.Status, campaign.Type, campaign.IsForcedVariationEnabled,campaign.IsBucketingSeedEnabled, campaign.Segments, campaign.Variables, campaign.isOB, campaign.isOBv2)
            {
