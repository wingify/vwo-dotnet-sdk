...
namespace VWOSdk
{
    internal class CampaignHelper
    {
        ...
        /// </returns>
        public static string getBucketingSeed(string userId, Campaign campaign, int? groupId, bool isNewBucketingEnabled = false,  bool isNewBucketingv2Enabled = false)
        {
            if (groupId != null)
            {
            ...
            if (campaign != null && (isNewBucketingEnabled || isNewBucketingv2Enabled || campaign.IsBucketingSeedEnabled))
            {
        ...
        public static bool checkForStorageAndWhitelisting(IVariationAllocator _variationAllocator, UserStorageAdapter _userStorageService, ISegmentEvaluator _segmentEvaluator,
           ...
           Dictionary<string, dynamic> customVariables, bool isNewBucketingEnabled = false,  bool isNewBucketingv2Enabled = false, Dictionary<string, dynamic> userStorageData = null, bool disableLogs = false)
        {
            ...
            foreach (BucketedCampaign campaign in campaignList)
            {
                ...
                Variation variation = _variationAllocator.TargettedVariation(userId, campaign, whiteListedVariations, isNewBucketingEnabled, isNewBucketingv2Enabled);
                if (variation != null)
                {
